...
const FilterSort = ({width, filterable, filtered, sortable, sorted, onClick}) => {
  if ((!filterable) && (!sortable)) {
    return null
  }

  const colors= {
    filter   : filterable ? filtered===true ? COLORS.on : COLORS.off : COLORS.disabled,
    sort_asc : sortable ? sorted==='asc' ? COLORS.on : COLORS.off : COLORS.disabled,
    sort_desc: sortable ? sorted==='desc' ? COLORS.on : COLORS.off : COLORS.disabled
  }

  return (
    <div className = {`axt-header-field-action`}
         onClick   = {onClick}>
      <svg version = "1.0"
        ...
        <g transform  = {`translate(0,512) scale(0.1, -0.1)`}
          ...
          <path fill={colors.filter}
                d="M2295 5114 c-596 -29 -1047 -99 -1348 -208 -169 -61 -284 -134 -336
          ...
          <path fill={colors.sort_desc}
                d="M844 2186 c-18 -14 -19 -35 -24 -639 l-5 -624 -260 103 c-143 57
          ...
          <path fill={colors.sort_asc}
                d="M4012 2183 c-16 -14 -711 -916 -801 -1039 -19 -26 -21 -35 -12 -53 7
